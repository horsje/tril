...
VI_NULL = 0
visa_dll = ctypes.windll.visa32


def chk(rslt):
    if rslt != 0:
        abbr, message = messages.get(rslt, ('', rslt))
        raise IOError("Visa call failed with error %s: %s" % (rslt, message))


class Visa(object):
    def __init__(self, visa_address):
        self.rm = ctypes.c_int(0)
        chk(visa_dll.viOpenDefaultRM(ctypes.byref(self.rm)))
        self.vi = ctypes.c_int(0)
        chk(visa_dll.viOpen(self.rm, visa_address.encode(), VI_NULL, VI_NULL, ctypes.byref(self.vi)))

    def __del__(self):
        if self.vi:
            chk(visa_dll.viClose(self.vi))
        if self.rm:
            chk(visa_dll.viClose(self.rm))

    def write(self, command):
        bytes_written = ctypes.c_int(0)
        chk(visa_dll.viWrite(self.vi, bytes(command + chr(10), 'ascii'), len(command) + 1, ctypes.byref(bytes_written)))

    def read(self):
        buf_size = 512
        response = ctypes.create_string_buffer(b'\0' * buf_size)
        bytes_read = ctypes.c_int(0)
        chk(visa_dll.viRead(self.vi, ctypes.byref(response), buf_size, ctypes.byref(bytes_read)))
        return response.value.decode()

    def clear(self):
        chk(visa_dll.viClear(self.vi))
